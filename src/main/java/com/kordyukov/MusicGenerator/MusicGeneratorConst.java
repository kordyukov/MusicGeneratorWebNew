package com.kordyukov.MusicGenerator;

public class MusicGeneratorConst {
    public static final int BASS = 33;
    public static final int BASS_NOTE_TONIC = 48;
    public static final int CHANNEL_BASS = 0;
    public static final int KICK = 36;
    public static final int CHANNEL_KICK = 9;
    public static final int PIANO = 1;
    public static final int CHANNEL_PIANO = 10;
    public static final int SNARE = 38;
    public static final int CHANNEL_SNARE = 9;
    public static final int HAT = 42;
    public static final int CHANNEL_HAT = 9;

    public static final String pathIdea = "src";
    public static final String pathTomcat = "webapps";

    public static final String baseDirTomcat = "webapps/MusicGenerator-0.0.1-SNAPSHOT/WEB-INF/classes/templates/MusicGenerator.wav";
    public static final String baseDirIdea = "src/main/resources/templates/MusicGenerator.wav";

}
